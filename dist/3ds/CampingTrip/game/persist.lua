...
function Persist.encode(data)
  ...
  for i, h in ipairs(data.history or {}) do
    ...
    hist[#hist + 1] = string.format(
      '{"destinationId":"%s","fruit":%d,"coffee":%d,"fish":{"ayu":%d,"trout":%d,"carp":%d}}',
      h.destinationId or "forest", h.fruit or 0, h.coffee or 0, hf.ayu or 0, hf.trout or 0, hf.carp or 0
    )
  ...
  if last then
    ...
    lastJson = string.format(
      '{"destinationId":"%s","fruit":%d,"coffee":%d,"fish":{"ayu":%d,"trout":%d,"carp":%d}}',
      last.destinationId or "forest", last.fruit or 0, last.coffee or 0, lf.ayu or 0, lf.trout or 0, lf.carp or 0
    )
  end
  return string.format(
    '{"castId":%d,"castChosen":%s,"lastDestinationId":"%s","trips":%d,"totals":{"coffee":%d,"fruit":%d,"fishTotal":%d,"fish":{"ayu":%d,"trout":%d,"carp":%d}},"lastTrip":%s,"history":[%s]}',
    data.castId or 1,
    data.castChosen and "true" or "false",
    data.lastDestinationId or "forest",
    data.trips or 0,
...
function Persist.load()
  ...
  saveData.lastDestinationId = "forest"
  saveData.trips = 0
  ...
  local lastDestinationId = raw:match('"lastDestinationId"%s*:%s*"([%w_-]+)"') or "forest"
  ...
  saveData.lastDestinationId = Destinations.get(lastDestinationId).id
...
function Persist.commitTrip()
  ...
  local snap = {
    destinationId = State.trip.destinationId or "forest",
