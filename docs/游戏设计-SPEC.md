...
| **下屏 320×240** | 菜单：`开始旅程` / `继续`（有存档时） / `装备图鉴` / `关于`；触摸或十字键选择，A 确认。底图中央保留浅色菜单安全区，装饰只落在边缘；不烘焙无文案的标题条或空标题框。 |
...
玩家可见完整一局（**首次**）=
**标题 → 序章 → 选角色 → 选择目的地 → 出发 → 营地 → 回家 → 书桌日记（存档）→ 标题**。
...
flowchart TD
  ...
  Prologue -->|已有角色| Destination[选择目的地]
  Cast -->|确认| Destination
  Destination -->|林间 / 海边| Depart[出发过渡]
...
| 4 | `destination` | **选择目的地** | 当前目的地 320×180 预览 | 林间营地 / 海边营地；A 确认 |
| 5 | `depart` | **出发过渡** | 各目的地文生图 2 张 | 按 A |
| 6 | `play` | 营地本体 | 由 Destination Pack 构建 | 背包 / 互动 / 本趟收获条 |
| 7 | `homecoming` | **回家过渡** | 文生图 1～2 张 + 短结算句 | 按 A 进入日记 |
| 8 | `diary` | **书桌日记（存档）** | 日记本/书桌分镜 | 本趟收获 + 累计；A 保存并回标题 |
...
| P3 | `story_p3_door.png` | 「去远一点的地方，过个周末吧。」 |
| P3 末 | 出门 | 进入选人（首次）或目的地选择 |
...
- 上屏：**TN 窄高手帐**叠在书桌氛围上（`diary_desk` + `diary_tn`；见 `docs/日记手帐-SPEC.md`）。左页保留墨色笔迹，右页只放旅行照片/票根/植物压花等纸面元素；禁止叠加站立角色 sprite，禁止半透明大黑条挡正文。
...
- 中文：真机 `chinese` 系统字体；桌面 `game/fonts/zh-ui.ttf`。`build-3ds-textures.py`
  会先扫描 `game/**/*.lua` 中的字符串并自动重建、逐字验证字体子集；缺少完整字体源或任一
  字形时构建直接失败，禁止再靠人工记忆补字。
...
| **DEV-093** | 2026-08-31 | done | **CIA 单变量实验包**：重打 55.3MB `CampingTrip.cia`；预检修正标题迁移后 `assets/ui/` 路径，并新增显式 `--allow-cia-experiment`（默认安全策略不变）；电脑侧 48/48 PASS |
| **DEV-094** | 2026-08-31 | done | **日记重写**：上屏不再挤真实文字，只画抽象像素笔迹；下屏改为「周末片段/当时心情/下次期待」三页感想，左右翻页、A 保存，移除数量流水账 |
| **DEV-095** | 2026-08-31 | done | **Old 3DS 最小 CIA**：RSF 改为 Legacy/268MHz，关闭 L2/Core2；换用 Title ID `000400000F4C4A00` 隔离旧缓存；首轮关闭 banner/audio；电脑侧 52/52 PASS |
| **DEV-096** | 2026-09-01 | done | **故事资源图谱**：新增独立 React Flow 本地工具；53 个剧情/仪式对象、66 条关系、470 个稳定编号资源对象；营地下屏六件装备及手冲 7/泡茶 7/钓鱼 6/品杯 2/做饭 5 步全部入图，选项保留在步骤卡内；节点可拖拽并按层级保存布局；支持预览、Finder 定位和遗漏审计 |
| **DEV-097** | 2026-09-01 | done | **PHASE-003 滤杯 ECSP 重切**：通用 `gen_slice_common.py` 新增边缘连通保主体切图；按真实主体间隔分组，不再等宽切片；五张滤杯等比缩放并保留 3px 透明安全边距，完整保留把手；同步重建 T3X |
| **DEV-098** | 2026-09-01 | done | **出发分镜差异化**：BEAT-004 改为封闭纵深的林间小径，BEAT-005 改为豁然开朗、尚未搭帐篷的溪边空地；两张按件文生图并登记制作血缘，修正旧图构图相似及“未搭先有帐篷”的叙事矛盾 |
| **DEV-099** | 2026-09-01 | done | **RES-0255 绿幕重制**：银针茶包改用纯绿幕文生图与 CKE 色键抠图，主体禁用绿色；替代会误删浅色包装及叶片图案的 BKE，并保留 3px 透明安全边距；同步重建 PNG/T3X 与 Story Atlas 血缘 |
| **DEV-100** | 2026-09-01 | done | **PHASE-011 温杯烫壶重制**：用“水壶注水＋盖碗＋蒸汽”的强轮廓绿幕图替换 239 字节程序占位图，CKE 输出 48×48；Story Atlas 的仪式/装备图改用完整适配，避免方形图被 `cover` 放大裁残 |
| **DEV-101** | 2026-09-01 | done | **营地帐篷比例与遮挡**：展开态恢复 48×40 原像素绘制（有效轮廓约 46×35px / 2.9×2.2 格），按实体底边落地；保留脚点 Y-sort，使人物在后方被遮挡、前方显示在前；底边三格设为碰撞 footprint，并新增前后截图与穿模断言 |
| **DEV-102** | 2026-09-01 | done | **非茶上屏分辨率闸门**：目录 icon 与 TOP preview 分离；手冲/钓鱼/品杯/做饭/图鉴/选角从高分辨率制作源直接构建 320×180；运行时单预览纹理缓存 ≤512 KiB；Story Atlas 标注资源角色；全屏故事分镜保持 400×240 有效区不变 |
| **DEV-103** | 2026-09-01 | done | **泡茶全流程高清双资源层**：PHASE-008~014 的下屏目录图保持 48×48，上屏改为制作源直出 256×192 / 320×180；温杯烫壶重做为清晰水壶注入盖碗动作；删除重复 PHASE-013，PHASE-014 保持稳定 ID并改为 6/6 |
| **DEV-104** | 2026-09-01 | done | **日记纸面去角色**：确认人物来自 `StoryDraw.diaryTop()` 运行时叠层而非背景烘焙；移除当前角色行走 sprite 与相框，PAGE-001/002/003 共用纯手帐上屏；playtest 增加无角色叠层断言与三页截图 |
| **DEV-105** | 2026-09-01 | done | **营地帐篷高清双资源层**：背包保留 48×40 `gear/tent.png` icon；新增纯绿幕文生图经 CKE 直出的 96×72 `tent_open_hd.png`，有效 bbox 92×68，以 0.5× 显示为 46×34px；保持脚点 Y-sort、前后遮挡及三格底边碰撞，并登记 Story Atlas 血缘 |
| **DEV-106** | 2026-09-01 | done | **RES-0446 标题下屏去空标题条**：确认 SCN-001 菜单顶部无文案后，以完整画幅 16-bit 文生图重制 320×240 有效区；顶部恢复为地图纸场景，中央保留五项菜单安全区；移除运行时旧遮片，登记制作血缘并重建 T3X。 |
| **DEV-107** | 2026-09-01 | done | **SCN-003 自然溪岸重制**：根因是旧浅水卵石 sheet 压缩成青白马赛克、岸线阈值抠图残留符号片段，并同时烘到水陆两侧形成一格宽高对比地毯。改用高分辨率深/浅水与洋红幕草土岸源，CKE 保绿、共享无缝边界、8 向陆侧 2–3px 岸唇；水格按空间变体打散重复，保持河道、踏脚石、码头和涉水碰撞，并修复移动链路未调用既有 `spawnSplash` 的失联水花；同步静态底图/T3X、Story Atlas 血缘与布局实例。 |
| **DEV-108** | 2026-09-02 | done | **手冲 PHASE-003/004/006 独立绿幕重制**：五款滤杯、滤纸、2/3/4 次分段注水共 9 个按件 1024² 纯绿幕源；CKE 分别直出 48×48 icon 与源直降 320×180 preview，统一 3px 安全边距；旧木架组合源、混合滤纸冲次源及程序占位不再是 active 上游。 |
| **DEV-109** | 2026-09-02 | done | **海边第二目的地端到端落地**：新增 Destination Pack registry，forest/coast 共用移动、装备、仪式与时钟；角色后加入目的地选择，旧存档默认 forest、行程记录目的地；文生图构建海边分镜、沙滩/海水/海岸松/灌木/礁石/漂木/海鸟/螃蟹与静态底图；海边具日出/白天/日落/夜晚状态；Story Atlas 增至 66 节点并物化双目的地六组；双路线 playtest、像素与 T3X 审计通过。 |
| **DEV-110** | 2026-09-02 | done | **海边生态与浪面可读性修正**：海面由一整条 400×64 文生图浪带替代深浅浪型混排的 16px 棋盘；删除静态模式逐格闪点及等距白条叠层；海鸥缩为约 18×14px、1× 绘制并移至潮线礁石；螃蟹扩大为约 34×18px，强化双钳、足和深色轮廓；独立制作源、血缘与 T3X 同步更新。 |
| **DEV-111** | 2026-09-02 | done | **玩家文案字体自动闸门**：新增 `build-font-subset.py`，忽略 Lua 注释并提取短字符串/长字符串，汇总全部运行时玩家文案及代码绘制符号；从完整中文字体自动生成 `zh-ui.ttf` 并逐字校验。`build-3ds-textures.py` 与部署链路强制先运行，字体源或输出缺任一字形即失败；当前 36 个 Lua 文件、697 个字符全部覆盖。 |
| **DEV-112** | 2026-09-02 | done | **海边独立海浪环境音**：用户提供 8分14秒 shoreline 原音，选取 270–316 秒较稳定段落，以 1 秒首尾交叉拼接输出 45 秒无缝循环并整体降低 6dB；桌面使用 48kHz OGG，3DS 使用 22050Hz mono 64k MP3；Destination Pack 将 coast 路由到 ocean，营地全域低垫、近潮线增强，forest 继续使用 creek。 |
| **DEV-113** | 2026-09-02 | done | **海上日出/日落视觉分层**：保留同一沙滩布局，新增两条 400×64 文生图时段海面覆盖层；日出为左侧淡金太阳、冷蓝紫海面与窄碎金倒影，日落为右侧橙红太阳、靛紫海面与宽铜色倒影，替代程序矩形太阳/直线反光；道具影子随光源左右换向，日出海鸥多螃蟹少、日落海鸥归巢且螃蟹上限提高，海浪音量黄昏略增强。 |
