...
### 3.0.5 书桌日记 = 存档（DEV-050 · DEV-071 TN）

- 回家分镜结束后进入 `diary`，不要直接回标题。  
- 上屏：**TN 窄高手帐**叠在书桌氛围上（`diary_desk` + `diary_tn`；见 `docs/日记手帐-SPEC.md`）。正文写在左页墨色，右页可贴角色小照；禁止半透明大黑条挡正文。  
- 下屏：便签式本趟摘要（果 / 鱼 / 咖啡 / 茶）+ 累计；「按 A 保存并回标题」。  
...
| **DEV-068** | 2026-08-31 | done | **代码模块化 P0–P8**：见 [代码架构-SPEC.md](./代码架构-SPEC.md)；`scene_flow` / `scenes/*` / `draw/*` / `input` / `runtime` / `session` / `bindings` 落地；main 从约 3700 行降至 **119 行 / 6 locals**；41 步 playtest PASS |
| **DEV-069** | 2026-08-31 | wip | **真机音频扩展**：`console_prox_amb_bgm_switch`、title/night BGM、近水溪水、近鸟 sfx；见 [真机音频扩展-SPEC.md](./真机音频扩展-SPEC.md) |
| **DEV-070** | 2026-08-31 | done | **帐篷 B 档选配 + 杯子品尝加深 + 钓鱼美术 v2**：`tent_gear.lua`（色×款×门帘→地图贴图）；`cup_sip.lua`（杯型口感缀句 + sip_1/2 特写）；钓点/仪式帧文生图重切；SPEC 见 `帐篷模块`/`杯子品尝` |
| **DEV-071** | 2026-08-31 | done | **日记 TN 窄高手帐**：文生图书桌底 + 打开本；`diary_desk`/`diary_tn` 叠层；左页墨水写本趟句、右页贴角色小照；去掉半透明黑条；SPEC 见 `日记手帐-SPEC.md` |
| **DEV-072** | 2026-08-31 | done | **装备横向拉齐**：帐篷升 A（pitch×3+mood）；扇子→做饭 A（菜系×火候×调味+三帧）；杯子升 A（一口×关注+sip×3）；新增评价 skill `linjian-gear-parity` |
| **DEV-073** | 2026-08-31 | done | **泡茶选参文字可读**：选茶/茶器等槽内只 `printf` 名称、备注改底栏；茶/手冲/钓鱼同布局；茶叶·茶器改**按件文生图**（袋面不烘焙汉字）；标题步数用 `·`；扩 `zh-ui` |
| **DEV-074** | 2026-08-31 | done | **帐篷地图看不见**：沙褐帐布被 `kill_bg` 木纹掩码抠没，地上只剩绿垫+细杆；`build-tent-assets` 改奶油/草垫专用抠图；营地贴图放大约 2.4 格 |
| **DEV-075** | 2026-08-31 | done | **泡茶特写比例**：注水等帧按件 4:3 文生图 + 上屏等比缩放（不再压 120×76）；**删除**杯子「一口大小」步骤，品尝从关注点起 |
| **DEV-076** | 2026-08-31 | done | **手帐纸页被抠穿**：`diary_tn` 改绿幕文生图 + 只抠 chroma green；右页小照改行走立绘避免白底块 |
| **DEV-077** | 2026-08-31 | done | **手冲水温 92° 看不见**：旧 PIL 浅蓝水壶叠在选中黄槽上几乎隐形；`temp_92/100` 改文生图金属壶（蓝带/沸红）+ 重建 t3x |
| **DEV-078** | 2026-08-31 | done | **手冲特写偏上**：`drip_1..3` 曾塞进 128² 上半（约 120×76）导致画面悬空；改 160×120 铺满 4:3 + 上屏真居中绘制 |
| **DEV-079** | 2026-08-31 | done | **杯子关注点/抬杯切图重做**：废 sheet 错切；按件文生图 `focus_*` + `sip_1..3`（160×120）；上屏居中；删残留 `sip_size_*` |
| **DEV-080** | 2026-08-31 | done | **手冲特写再下移**：按 160×120 内容区 + `drawFitted`；标题下留白带内偏下排；裁切略偏天空让注水落点更居中 |
| **DEV-081** | 2026-08-31 | done | **做饭选参按件出图**：菜系/火候/调味不再 sheet 竖条错切；`build-cook-assets.py` 一件一图；修空 `cuisine_sushi.t3x` |
| **DEV-082** | 2026-08-31 | done | **帐篷占一格**：地图绘制从 `TILE×2.4`（且 scale≥1）改为 contain 进单格 16×16 |
| **DEV-083** | 2026-08-31 | done | **帐篷变两块绿空地**：帐底软绿垫未抠净 + 可能残留多格 tile=5；加强 `kill_sheet_pad`、重建贴图、放置时清光所有帐篷格 |
| **DEV-084** | 2026-08-31 | done | **研磨度抠穿**：改按件绿幕文生图 + `kill_chroma_green`，不再用奶油 sheet 误伤粉堆 |
| **DEV-085** | 2026-08-31 | done | **品尝特写去人物**：`sip_1..3` 只拍杯子（抬/入口/放下），避免选女生却出男孩 |
| **DEV-086** | 2026-08-31 | done | **做饭全按件出图**：菜系绿幕重抠；`cook_1..3` 独立 160×120（开火/翻面/盛盘），不再 sheet 截半截 |
| **DEV-087** | 2026-08-31 | done | **手帐换行可读** + **钓鱼特写无人物**：左页墨水自动换行；`fish_1..4/miss` 按件 160×120（无角色），避免截取半截男孩 |
| **DEV-088** | 2026-08-31 | done | **返回二次确认**：序章～日记中 B/Esc 先提示「返回将丢失本趟进度」；默认「留下」，确认「返回」才回标题；图鉴/关于仍直接返回 |
| **DEV-089** | 2026-08-31 | done | **帐篷下绿方块**：tile=5 地面误走草地分支；改为沿用搭帐前土地（默认营地泥地），收帐恢复原格而非写成草地 |
| **DEV-090** | 2026-08-31 | done | **playtest 扩展 + 修启动**：补回缺失 `scenes/codex.lua`；手帐 `diaryAudit`（禁省略号/换行丢字/缺行）；帐篷地面、返回确认断言；步骤 0–42 |
| **DEV-091** | 2026-08-31 | done | **帐篷简化**：移除色/款/门帘及三步搭建；唯一米白尖顶帐篷按 A 直接展开/收起；地图与装备格共用透明 PNG，视觉放大至约 1.75 格 |
| **DEV-092** | 2026-08-31 | done | **帐篷悬空**：源 PNG 实体底边为 y=37/40，不能按透明画布底部对齐；改按不透明实体底边落地并下压 1px |
